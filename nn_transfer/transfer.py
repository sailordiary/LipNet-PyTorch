...
def convert_lipnet(pytorch_model, path, flip_filters=None, verbose=True):
    ...
    with h5py.File(path, 'r') as f:
        ...
        for layer in pytorch_layer_names:
            if layer == 'conv.0':
                params = util.dig_to_params(model_weights['conv1'])
            elif layer == 'conv.4':
                params = util.dig_to_params(model_weights['conv2'])
            elif layer == 'conv.8':
                params = util.dig_to_params(model_weights['conv3'])
            else:
                continue
            ...
            if weight_key in input_state_dict:
                ...
                print ('Loading layer:', layer, 'Params:', weights.shape)
    ...
    print ('Weight transfer and conversion complete.')
